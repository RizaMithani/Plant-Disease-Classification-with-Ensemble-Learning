...
if torch.cuda.is_available():
    resnet50 = mobilenet.cuda()

# Defining loss and optimizer
criterion = nn.CrossEntropyLoss()
optimizer = AdaBelief(mobilenet.parameters(), lr=1e-3, eps=1e-8, betas=(0.9,0.999), weight_decouple=True, rectify=True)
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'checkpoint_mada.pt')
...
model, train_losses, val_losses, train_accuracies, val_accuracies = train_model(mobilenet, train_loader, val_loader, optimizer, criterion, n_epochs, early_stopping, scheduler=exp_lr_scheduler)
print("Training Complete", flush=True)

torch.save(model.state_dict(), 'plant_village_mobilenet_ada_model.pth')
...
plt.legend()
plt.tight_layout()
plt.savefig('loss_plot_mobilenet_ada.png')  
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('accuracy_plot_mobilenet_ada.png')
plt.show()
...
plt.tight_layout()
plt.savefig('confusion_matrix_mobilenet_ada.png', bbox_inches='tight') 
plt.show()
