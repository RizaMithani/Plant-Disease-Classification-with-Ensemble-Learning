...
efficientnet = timm.create_model('efficientnet_b0', pretrained=True, num_classes=len(train_dataset.classes))
...
optimizer = AdaBelief(efficientnet.parameters(), lr=1e-3, eps=1e-8, betas=(0.9,0.999), weight_decouple=True, rectify=True)
...
model, train_losses, val_losses, train_accuracies, val_accuracies = train_model(efficientnet, train_loader, val_loader, optimizer, criterion, n_epochs, early_stopping, scheduler=exp_lr_scheduler)
print("Training Complete", flush=True)

torch.save(model.state_dict(), 'plant_village_efficientnet_ada_model_og.pth')
...
plt.legend()
plt.tight_layout()
plt.savefig('loss_plot_efficientnet_ada_og.png')  
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('accuracy_plot_efficientnet_ada_og.png')
plt.show()
...
plt.tight_layout()
plt.savefig('confusion_matrix_efficientnet_ada_og.png', bbox_inches='tight')  
plt.show()
