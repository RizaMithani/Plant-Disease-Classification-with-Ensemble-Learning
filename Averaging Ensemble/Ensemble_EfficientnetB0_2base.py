...
train_loader1 = DataLoader(train_dataset1, batch_size=32, shuffle=True, num_workers=4)
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'checkpoint_eada.pt')
...
model_out1, train_losses1, val_losses1, train_accuracies1, val_accuracies1 = train_model(model1, train_loader1, val_loader, optimizer1, criterion1, n_epochs, early_stopping1, scheduler=exp_lr_scheduler1)
print("Training Complete for Model 1", flush=True)

print("Training Started for Model 2", flush=True)
model_out2, train_losses2, val_losses2, train_accuracies2, val_accuracies2 = train_model(model2, train_loader2, val_loader, optimizer2, criterion2, n_epochs, early_stopping2, scheduler=exp_lr_scheduler2)
print("Training Complete for Model 2", flush=True)

def ensemble_predictions(model1, model2, dataloader, criterion):
    all_predictions = []
    all_labels = []
    total_loss = 0.0
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            # Average the outputs
            averaged_outputs = (outputs1 + outputs2) / 2
...
val_predictions, val_labels, val_loss = ensemble_predictions(model_out1, model_out2, val_loader, criterion1)
test_predictions, test_labels, test_loss = ensemble_predictions(model_out1, model_out2, test_loader, criterion2)
