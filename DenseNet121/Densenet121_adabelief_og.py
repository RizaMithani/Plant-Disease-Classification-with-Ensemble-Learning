...
optimizer = AdaBelief(densenet.parameters(), lr=1e-3, eps=1e-8, betas=(0.9,0.999), weight_decouple=True, rectify=True)
...
model, train_losses, val_losses, train_accuracies, val_accuracies = train_model(densenet, train_loader, val_loader, optimizer, criterion, n_epochs, early_stopping, scheduler=exp_lr_scheduler)
print("Training Complete", flush=True)

torch.save(model.state_dict(), 'plant_village_densenet_ada_model_og.pth')
...
plt.legend()
plt.tight_layout()
plt.savefig('loss_plot_densenet_ada_og.png')  
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('accuracy_plot_densenet_ada_og.png')
plt.show()
...
plt.tight_layout()
plt.savefig('confusion_matrix_densenet_ada_og.png', bbox_inches='tight')  
